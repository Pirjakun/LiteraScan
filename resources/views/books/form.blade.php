        ...
        <form action="{{ $book->exists ? route('books.update', $book) : route('books.store') }}" method="POST" class="flex flex-col gap-5 relative">
            ...
            <div class="flex flex-col gap-2">
                ...
                <input type="text" name="title" id="title" value="{{ old('title', $book->title) }}" required placeholder="Contoh: Dongeng Si Kancil"
                       class="w-full px-4 py-3 rounded-2xl bg-slate-50 border border-slate-200 text-slate-800 placeholder-slate-400 text-sm focus:outline-none focus:border-sky-mid focus:bg-white transition-all">
            </div>
            ...
            <div class="grid grid-cols-1 @if($book->exists) md:grid-cols-2 @endif gap-5">
                <div class="flex flex-col gap-2">
                    ...
                    <input type="text" name="rfid_uid" id="rfid_uid" value="{{ old('rfid_uid', $book->rfid_uid) }}" required placeholder="Contoh: 04 E5 D7 08 C1 2A 81"
                           class="w-full px-4 py-3 rounded-2xl bg-slate-50 border border-slate-200 text-slate-800 placeholder-slate-400 text-sm font-mono focus:outline-none focus:border-sky-mid focus:bg-white transition-all">
                ...
                @if($book->exists)
                    <div class="flex flex-col gap-2">
                        ...
                        <select name="status" id="status" required
                                class="w-full px-4 py-3 rounded-2xl bg-slate-50 border border-slate-200 text-slate-800 text-sm focus:outline-none focus:border-sky-mid focus:bg-white transition-all">
            ...
            <div class="flex items-center justify-end gap-3 mt-3 pt-5 border-t border-slate-100">
                ...
                <button type="submit" class="px-6 py-2.5 bg-grape-mid hover:bg-grape-deep text-sky-deep rounded-full text-sm font-bold transition-all shadow-lg shadow-grape-mid/50">
