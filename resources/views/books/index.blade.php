...
@section('title', 'Manajemen Buku - LiteraScan')
    ...
    <div class="flex flex-wrap gap-3 justify-between items-center">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-xs text-slate-400">Kelola daftar buku perpustakaan beserta label RFID-nya</p>
            </div>
        </div>
        <a href="{{ route('books.create') }}" class="px-5 py-2.5 bg-gradient-to-r from-peach-deep to-rose-deep hover:opacity-90 text-white rounded-full text-sm font-bold transition-all shadow-lg shadow-peach-mid/40 flex items-center gap-2">
                    ...
                    <tr class="hover:bg-slate-50 transition-all">
                        ...
                        <td class="p-4">
                            <div class="flex items-center justify-center gap-2">
                                ...
                                <form action="{{ route('books.destroy', $book) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus buku ini?');" class="inline">
                                    @csrf
                                    @method('DELETE')
